...
- `add`：新增任务
- `list`：列出任务
- `filter`：按 source、关键词、完成状态、优先级范围筛选
- `show`：查看单个任务的完整详情
- `update`：更新任务，支持追加备注
- `delete`：删除任务
...
```bash
go run ./cmd/gtask add --title "write docs" --priority 2 --source aistudio --days 3 --meta '{"kind":"draft"}' --note "first note"
go run ./cmd/gtask add --title "night run" --target "2026-04-20 21"
go run ./cmd/gtask list
go run ./cmd/gtask filter --source idea1 --query CDP
go run ./cmd/gtask show 4
go run ./cmd/gtask update 1 --completed true --note "done locally"
